...
const Learning = ({ isAuthenticated }) => {
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      try {
        const subjectResponse = await axios.get("http://localhost:5000/api/subjects", { withCredentials: true });
        const topicResponse = await axios.get("http://localhost:5000/api/topics", { withCredentials: true });
